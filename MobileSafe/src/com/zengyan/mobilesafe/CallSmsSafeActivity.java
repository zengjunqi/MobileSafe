...
public class CallSmsSafeActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		llloadLayout = (LinearLayout) findViewById(R.id.ll_load);
	...
	private void getData() {
		new Thread() {
			...
			public void run() {
				if (infos==null) {
					infos = dao.findPart(offset, maxno);
				}else {
					infos.addAll(dao.findPart(offset, maxno));
				}
				
				// infos = dao.findAll();
				runOnUiThread(new Runnable() {
					public void run() {
						llloadLayout.setVisibility(View.INVISIBLE);
						if (adapter==null) {
